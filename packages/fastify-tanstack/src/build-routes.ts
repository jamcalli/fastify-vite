...
export type RenderMode = 'stream' | 'full' | 'buffered'

// Render flags ride route config because request.routeOptions hides custom keys
...
// `routeOptions` export from a `.server.ts` sibling; renderer owns url/handler
export type SiblingRouteOptions = Omit<Partial<RouteOptions>, 'url' | 'handler'> & {
  // 'stream' (default) matches TanStack's defaultStreamHandler; 'full' waits for the
  // whole tree; 'buffered' also swaps script-src nonces for hashes so shared caches can hold it
  render?: RenderMode
}
...
export function normalizeConfigKey(globKey: string, routesDir = 'routes'): string | null {
  ...
  // Nested index route ids keep a trailing slash, e.g. _app/index -> /_app/
  ...
  const visit = (node: AnyNode, ancestorStack: RoutePlugin[]) => {
    ...
    if (isEmitted) {
      emitted.push({
        ...
        routeOptions: sibling?.routeOptions,
      })
    }
...
function extractSibling(mod: unknown, filename: string): ResolvedSibling {
  ...
  // Without skip-override, scope.register() isolates the plugin's hooks in a child scope and they silently never fire
  const plugin = raw as RoutePlugin & { [skipOverride]?: boolean }
  plugin[skipOverride] = true
  const routeOptions = record.routeOptions
  if (routeOptions !== undefined && (typeof routeOptions !== 'object' || routeOptions === null)) {
    throw new Error(`${filename} export 'routeOptions' must be a plain object`)
  }
  return {
    plugin,
    cascadeHooks: record.cascadeHooks === true,
    routeOptions: routeOptions as SiblingRouteOptions | undefined,
  }
}
